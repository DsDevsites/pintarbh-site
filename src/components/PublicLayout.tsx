  ...
  return (
    <header className="sticky top-0 z-40 border-b border-zinc-100 bg-white/90 backdrop-blur-xl">
      ...
      <nav className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-3 px-4 py-2 md:min-h-[76px] md:px-8 lg:px-12">
        <Link to="/" aria-label="Página inicial" className="min-w-0" onClick={() => setOpen(false)}><Logo logoUrl={settings.logoUrl} compactMobile /></Link>
  ...
  return (
    <footer className="bg-zinc-950 text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 md:grid-cols-[1.3fr_1fr_1fr] lg:px-8">
        <div><Logo logoUrl={settings.logoUrl} dark={false} /><p className="mt-5 max-w-sm text-sm leading-6 text-zinc-300">Pintura profissional, acabamentos e fachadas com atendimento cuidadoso em Belo Horizonte e região.</p></div>
      ...
      <div className="border-t border-white/10 px-5 py-5 text-center text-xs text-zinc-400">© {new Date().getFullYear()} PintarBH. Todos os direitos reservados.</div>
    </footer>
  );
}
