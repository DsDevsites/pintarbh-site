  ...
  return (
    <header className="sticky top-0 z-40 border-b border-zinc-100 bg-white/90 backdrop-blur-xl">
      ...
      <nav className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-3 px-4 py-2 md:min-h-[76px] md:px-8 lg:px-12">
        <Link to="/" aria-label="Página inicial" className="min-w-0" onClick={() => setOpen(false)}>
        ...
        <button
          type="button"
          className="ml-auto grid h-11 w-11 shrink-0 place-items-center rounded-full border border-zinc-200 md:hidden"
      ...
      {open && (
        <div id="mobile-navigation" className="border-t border-zinc-100 bg-white px-4 py-3 md:hidden">
    ...
    <footer className="bg-zinc-950 text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 md:grid-cols-[1.3fr_1fr_1fr] lg:px-8">
        ...
        <div>
          ...
          <div className="mt-4 grid gap-3 text-sm text-zinc-300">
            <a href={settings.phone ? `tel:${settings.phone.replace(/\\D/g, '')}` : '#'} className="flex items-center gap-2 hover:text-white">
              <Phone className="h-4 w-4" />{settings.phone}
            </a>
            <a href={settings.email ? `mailto:${settings.email}` : '#'} className="flex items-center gap-2 hover:text-white">
            ...
            <div className="flex gap-3 pt-2">
              <a href={settings.instagram} aria-label="Instagram" target="_blank" rel="noopener noreferrer"><Instagram className="h-5 w-5" /></a>
              <a href={settings.facebook} aria-label="Facebook" target="_blank" rel="noopener noreferrer"><Facebook className="h-5 w-5" /></a>
            </div>
          </div>
        </div>
      </div>
