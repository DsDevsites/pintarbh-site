  ...
  if (settingsQuery.isError) {
    return (
      <div className="grid min-h-screen place-items-center bg-white px-5 text-center text-sm text-zinc-600">
        Não foi possível carregar o conteúdo do site. Atualize a página e tente novamente.
      </div>
    );
  }
          ...
          <div className="mx-auto max-w-7xl px-5 md:px-8 lg:px-12">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 md:gap-6 lg:grid-cols-3 lg:gap-8">
              {services.length ? services.map((service) => (
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 p-8 text-sm text-zinc-500 sm:col-span-2 lg:col-span-3">
                  Os serviços serão apresentados aqui em breve.
                </div>
              )}
            </div>
          </div>
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-white to-zinc-50 py-20">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mb-12 text-center">
              ...
              <h2 className="mt-6 text-4xl font-bold tracking-tight text-zinc-900 md:text-5xl">Conheça nossos projetos<br />ao som da PintarBH.</h2>
              <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-zinc-600">Enquanto você navega pelos nossos trabalhos, aproveite uma seleção de músicas preparada para acompanhar cada pintura, reforma e acabamento.</p>
            </div>
            <div className="grid items-center gap-10 lg:grid-cols-2">
              <div className="space-y-8">
                <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-xl">
                  <div className="mb-6 flex items-center gap-4">
                    ...
                    <div><h3 className="text-2xl font-bold">Playlist Oficial</h3><p className="text-zinc-500">O ritmo da PintarBH</p></div>
                  </div>
              ...
              <div className="overflow-hidden rounded-3xl border border-zinc-200 bg-white p-3 shadow-2xl">
                <iframe title="Playlist oficial da PintarBH no Spotify" style={{ borderRadius: '20px' }} src="https://open.spotify.com/playlist/1rAlWRRPcJfU2bUuESTlUQ?si=EH-SlHfCSQ2ZMkVEHO7EiQ&pi=fgLBHP-0T4yWu" width="100%" height="480" frameBorder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl px-5 md:px-8 lg:px-12">
            <motion.div {...fadeUp} className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
              <div>
                ...
                <h2 className="mt-4 text-3xl font-light md:text-5xl">Projetos realizados pela PintarBH.</h2>
                <p className="mt-4 max-w-2xl text-sm leading-7 text-zinc-600">Veja alguns trabalhos e conheça os detalhes de cada projeto.</p>
              </div>
            </motion.div>
            <div className="mt-8 grid gap-4 md:gap-6 lg:grid-cols-3 lg:gap-8">
              {projects.length ? projects.map((project) => (
                <motion.article {...fadeUp} key={project.id} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-zinc-200">
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.12em] text-zinc-500"><MapPin className="h-4 w-4" /> {project.location}</div>
                    <h3 className="mt-3 text-2xl font-semibold">{project.title}</h3>
                    <p className="mt-3 text-sm leading-6 text-zinc-600">{project.shortDescription}</p>
                    <Link to="/projeto/$slug" params={{ slug: project.slug }} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold">Ver detalhes <ArrowRight className="h-4 w-4" /></Link>
                  </div>
                </motion.article>
              )) : (
                <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-sm text-zinc-500 md:col-span-2 lg:col-span-3">
                  Os projetos serão apresentados aqui em breve.
                </div>
              )}
            </div>
          </div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3 md:gap-6 lg:gap-8">
              {testimonials.length ? testimonials.map((testimonial) => (
                <motion.article {...fadeUp} key={testimonial.id} className="rounded-2xl border border-zinc-200 p-6">
                  <div className="flex gap-1 text-amber-400">{Array.from({ length: testimonial.rating }).map((_, index) => <Star key={index} className="h-4 w-4 fill-current" />)}</div>
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 p-8 text-sm text-zinc-500 md:col-span-3">
                  Os depoimentos serão apresentados aqui em breve.
                </div>
              )}
            </div>
          </div>
